...
public class RNViewUtils {
    ...
    public static void saveScreenAndTitle(String screenName, String title) {
        ...
        try {
            ...
            screenProperties.put("H_title", title);
            screenProperties.put("H_screen_name", screenName);
    ...
    /**
     * 供可视化调用，返回 H_title，H_screen_name，勿删
     *
     * @return json 格式
     */
    public static String getVisualizeProperties() {
        //当前页面不可见或无 H_screen_name H_title 时，可视化获取原生页面信息
    ...
    private static void setCurrentActivity(Activity currentActivity) {
        ...
        if (properties != null && properties.has("H_screen_name")) {
            saveScreenAndTitle(properties.optString("H_screen_name"), properties.optString("H_title"));
        } else {
